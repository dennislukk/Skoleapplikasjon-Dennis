...
class DatabaseManager:
    ...
    def execute_query(self, query, params=(), commit=False):
        ...
        try:
            ...
            if commit:
                conn.commit()
                return True          # <-- now returns True on successful commit
            else:
...
class SchoolDatabaseApp:
    ...
    def sort_results(self):
        ...
        for r in rows:
            self.results_tree.insert('', 'end', values=r)
        self.status_var.set(f"{len(rows)} poster i '{tbl}' sortert etter {col}.")
        self.reselect_first_row()
    ...
    def add_record(self):
        ...
        if any(v == '' for v in vals):
            ...
            return
        if self.db_manager.execute_query(self.tables[tbl]['insert_query'], tuple(vals), commit=True) is None:
            return
    ...
    def update_selected_record(self):
        ...
        if self.db_manager.execute_query(q, tuple(vals), commit=True) is None:
            return
    ...
    def delete_records(self):
        ...
        for s in sels:
            ...
            if self.db_manager.execute_query(q, tuple(pkvals), commit=True) is None:
                return
