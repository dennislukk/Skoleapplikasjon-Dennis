...
class SchoolDatabaseApp:
    def __init__(self, root):
        ...
        ##########################################
        # Tabellkonfigurasjon med felt og fremmednøkler
        ##########################################
        self.tables = {
            "admin": {
                "fields": ["epost"],
    ...
    def refresh_results(self):
        """Henter og viser data for valgt tabell"""
    ...
    def create_main_layout(self):
        ...
        header_label = tk.Label(header_frame, text="Skole Database Administrasjon", bg='#2980b9', fg='white', font=("Arial", 20, "bold"))
        ...
        self.table_dropdown = ttk.Combobox(menu_frame, textvariable=self.table_var, values=list(self.tables.keys()), state="readonly", width=30)
    ...
    def create_status_bar(self):
        ...
        self.status_bar = tk.Label(self.root, textvariable=self.status_var, bd=1, relief=tk.SUNKEN, anchor='w', font=("Arial", 9), bg='white', fg='#2c3e50')
    ...
    def clear_form(self):
        for widget in self.entry_widgets.values(): widget.delete(0, tk.END)
    ...
    def on_table_select(self, event=None):
        ...
        if not tbl or not info: return
        for w in self.data_entry_frame.winfo_children(): w.destroy()
        self.entry_widgets = {}
        fks = info.get('foreign_keys', {})
        for field in info['fields']:
            frm = tk.Frame(self.data_entry_frame, bg='#dfe6e9'); frm.pack(fill=tk.X, padx=5, pady=3)
            tk.Label(frm, text=f"{field}:", width=15, anchor='w', bg='#dfe6e9', font=("Arial",10)).pack(side=tk.LEFT)
            if field in fks:
                ref = fks[field]; ref_info = self.tables[ref['table']]
                data = self.db_manager.execute_query(ref_info['select_query']) or []
                idxs = [ref_info['fields'].index(df) for df in ref['display_fields']]
                seen, vals = set(), []
                for row in data:
                    txt = " ".join(str(row[i]) for i in idxs)
                    if txt not in seen: seen.add(txt); vals.append(txt)
        ...
        self.sort_combobox['values'] = info['fields']; self.sort_var.set(info['fields'][0])
        self.populate_results_tree(tbl)
        self.status_var.set(f"Viser data for '{tbl}'.")

    def populate_results_tree(self, tbl):
        for item in self.results_tree.get_children(): self.results_tree.delete(item)
        cols = tuple(self.tables[tbl]['fields']); self.results_tree['columns'] = cols
        self.results_tree.heading('#0',text=''); self.results_tree.column('#0',width=0,stretch=tk.NO)
        for c in cols: self.results_tree.heading(c,text=c); self.results_tree.column(c,anchor='center',width=120)
        rows = self.db_manager.execute_query(self.tables[tbl]['select_query']) or []
        for r in rows: self.results_tree.insert('', 'end', values=r)
        self.status_var.set(f"{len(rows)} poster funnet i '{tbl}'."); self.reselect_first_row()

    def on_result_select(self, event=None):
        sels = self.results_tree.selection()
        if not sels:
            self.details_text.configure(state='normal'); self.details_text.delete('1.0', tk.END)
            self.details_text.insert(tk.END, "Ingen rad er valgt."); self.details_text.configure(state='disabled'); return
        vals = self.results_tree.item(sels[0])['values']; fields = self.tables[self.table_var.get()]['fields']
        self.details_text.configure(state='normal'); self.details_text.delete('1.0', tk.END)
        for f,v in zip(fields, vals): self.details_text.insert(tk.END, f"{f}: {v}\n")
        self.details_text.configure(state='disabled')

    def perform_search(self, event=None):
        tbl = self.table_var.get(); term = self.search_entry.get().lower().strip()
        if not tbl: return
        self.results_tree.delete(*self.results_tree.get_children())
        rows = self.db_manager.execute_query(self.tables[tbl]['select_query']) or []
        matches = [r for r in rows if any(term in str(val).lower() for val in r)]
        for m in matches: self.results_tree.insert('', 'end', values=m)
    ...
    def sort_results(self):
        ...
        if not tbl or not col: messagebox.showinfo("Sorter","Velg kolonne."); return
        q = self.tables[tbl]['select_query'] + f" ORDER BY {col} ASC"
        rows = self.db_manager.execute_query(q) or []
        self.results_tree.delete(*self.results_tree.get_children())
        for r in rows: self.results_tree.insert('', 'end', values=r)
        self.status_var.set(f"{len(rows)} poster i '{tbl}' sortert etter {col}."); self.reselect_first_row()
    ...
    def add_record(self):
        ...
        if any(v == '' for v in vals): messagebox.showwarning("Advarsel","Alle felt må fylles ut."); return
        if self.db_manager.execute_query(self.tables[tbl]['insert_query'], tuple(vals), commit=True) is None: return
        messagebox.showinfo("Suksess","Posten ble lagt til."); self.clear_form(); self.refresh_results()

    def populate_form_for_selected(self):
        sels = self.results_tree.selection()
        if not sels: messagebox.showwarning("Advarsel","Velg rad først."); return
        self.selected_item = sels[0]; tbl = self.table_var.get(); fields = self.tables[tbl]['fields']
        vals = self.results_tree.item(self.selected_item)['values']
        for i,f in enumerate(fields): w=self.entry_widgets[f]; w.delete(0,tk.END); w.insert(0,str(vals[i]))
        self.update_detail_box()

    def update_detail_box(self):
        self.details_text.configure(state='normal'); self.details_text.delete('1.0', tk.END)
        for f,w in self.entry_widgets.items(): self.details_text.insert(tk.END, f"{f}: {w.get()}\n")
        self.details_text.configure(state='disabled')

    def update_selected_record(self):
        tbl = self.table_var.get()
        if not tbl or not hasattr(self,'selected_item'): messagebox.showwarning("Advarsel","Velg og rediger rad først."); return
        fields = self.tables[tbl]['fields']; new= [self.entry_widgets[f].get().strip() for f in fields]
        if any(v=='' for v in new): messagebox.showwarning("Advarsel","Alle felt må fylles ut."); return
        ...
        if self.db_manager.execute_query(q, tuple(vals), commit=True) is None: return
        messagebox.showinfo("Suksess","Post oppdatert."); self.refresh_results()

    def update_all_records(self):
        tbl = self.table_var.get();
        if not tbl: messagebox.showwarning("Advarsel","Velg tabell først."); return
        vals = [w.get().strip() for w in self.entry_widgets.values()]
        if any(v==' ' for v in vals): messagebox.showwarning("Advarsel","Alle felt må fylles ut."); return
        ...
        if self.db_manager.execute_query(q,tuple(vals),commit=True) is None: return
        messagebox.showinfo("Suksess","Alle poster oppdatert."); self.refresh_results()

    def delete_records(self):
        tbl = self.table_var.get(); sels = self.results_tree.selection()
        if not tbl or not sels: messagebox.showwarning("Advarsel","Velg tabell og rader."); return
        if not messagebox.askyesno("Bekreft",f"Slette {len(sels)} poster?"): return
        fields = self.tables[tbl]['fields']; pk=self.tables[tbl].get('primary_key',fields)
        for s in sels:
            vals=self.results_tree.item(s)['values']; pkvals=[vals[fields.index(p)] for p in pk]
            q=f"DELETE FROM {tbl} WHERE {' AND '.join(p+'=%s' for p in pk)}"
            if self.db_manager.execute_query(q,tuple(pkvals),commit=True) is None: return
        messagebox.showinfo("Suksess",f"Slettet {len(sels)} poster."); self.refresh_results()
